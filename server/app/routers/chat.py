...
def send_message(conversation_id: int, body: MessageIn):
    ...
    # Stream the architect turn through the shared SSE bridge: keepalive while the model/tools
    # are silent, and a no-progress watchdog that surfaces an error rather than keepalive-ing
    # forever if the turn wedges (a ping-stalled model stream or an uncancellable hung tool).
    return sse.stream(
        architect.run(conversation_id, body.text, location, mode,
                      fresh_context=body.fresh_context, deep=body.deep),
        fail_message="Something went wrong while generating the reply. Please try again.",
        wedged_message="The assistant stopped responding. Please try again.",
        label="chat",
    )
