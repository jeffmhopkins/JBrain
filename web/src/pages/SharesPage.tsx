...
export default function SharesPage() {
  ...
  async function load() {
    setLoading(true);
    try { const r = await get<{ links: ShareLink[]; proposals: Proposal[] }>("/api/shares"); setLinks(r.links); setProposals(r.proposals); }
    catch { /* ignore */ }
    setLoading(false);
  }
  useEffect(() => { load(); }, []);
  ...
  async function accept(p: Proposal) {
    ...
    catch (e: any) { alert(e?.message || "Couldn't accept."); load(); }
  }
  async function reject(p: Proposal) {
    setProposals((ps) => ps.filter((x) => x.id !== p.id));
    try { await post(`/api/shares/proposals/${p.id}/reject`); } catch { load(); }
  }
          ...
          {proposals.map((p) => (
            <div className="card" key={p.id}>
              ...
              <button className="ghost" style={{ fontSize: 12 }} onClick={() => setOpenDiff(openDiff === p.id ? null : p.id)}>
                {openDiff === p.id ? "Hide" : "Show"} proposed content
              </button>
              {openDiff === p.id && <pre className="share-diff">{p.proposed_content}</pre>}
